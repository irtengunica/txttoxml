...
with open("testepoch300.json") as json_file:  
 ...
 for jsn in data['frameler']:
  ...
  if frame_idlist[dx]==jsn['frame_id']:
   ...
   ET.SubElement(annotation, "filename").text = str(resimadilist[jsn['frame_id']-1])
   ET.SubElement(annotation, "path").text = str(secilendosyalist[jsn['frame_id']-1])
   ...
   tree.write(str(secilendosyalist[jsn['frame_id']-1]).replace('.jpg','')+".xml")
    #else:
     #color = (255,0,0)
